...
class Duration {
  ...
  plus(duration) {
    ...
    for (const k of orderedUnits$1) {
      if (luxon_hasOwnProperty(dur.values, k) || luxon_hasOwnProperty(this.values, k)) {
...
function match(input, regex, handlers) {
  ...
  if (matches) {
    ...
    for (const i in handlers) {
      if (luxon_hasOwnProperty(handlers, i)) {
        const h = handlers[i],
    ...
    if (!this.isValid) {
      ...
      const [rawMatches, matches] = match(input, this.regex, this.handlers),
        [result, zone, specificOffset] = matches
          ...
          : [null, null, undefined];
      if (luxon_hasOwnProperty(matches, "a") && luxon_hasOwnProperty(matches, "H")) {
        throw new ConflictingSpecificationError(
...
// not used
function toICSLocalDateTimeString(date) {
...
function toFloatingLuxon(jsDate) {
